...
function raf($NomDuBouton,$TexteParDefaut=''){ // raf dans toute les funct
...
function afficherFormulaireCommentaire($Page,$IdArticle=0){
	...
	if($AfficherForm==1){
		?>
		<form action="<?php echo $Page; ?>" method="post" class='commentaireinput'>
  <div>
  </div>
  <div class='commentaireinput1'>
  <div>
  
			
			<input class='commentaireidk' type="text" name="pseudo" value="<?php echo raf("pseudo");echo $_SESSION['username'] ?>" maxlength="20" required="required" placeholder="Entrer votre Pseudo">
  </div>
    	<!--
  ...
  <div>
			...
			<input class='commentaireidk' type="text" name="mail" value="<?php echo raf("mail");?>" maxlength="50" required="required" placeholder="Entrer votre email">
  </div>
   
  <div>
 
			<textarea name="commentaire" rows="10" cols="50" required="required">
       <?php echo raf("commentaire"); ?>
			</textarea>
    			<!--
  ...
  <div>
			<input class='commentaireidk' type="submit" name="envoyer" value="Poster!">
  </div>
  </div>
		</form>
		<?php
...
function afficherCommentaires($IdArticle=0){
	...
	if(mysqli_num_rows($req)==0) {
		...
		?>
		<div class="commentaireinput">
  <div>
    
  </div>
  <div class="commentaireposte">
			<p class="borderbot"><b><?php echo $infos['pseudo']; ?> a posté le <?php echo date("d-m-Y",$infos['quand'])." à ".date("H:i",$infos['quand']); ?></b></p>
...
<?php
$footer='<footer class="footerall" style="">
<div style="display:flex;justify-content:space-between;margin-top:20px;border-top:1px solid #ADA99D;color:#ADA99D; ">
...
<?php
//gros streamer AMINEMATUE  OTPLOL_  MISTERMV  JLTOMY  KAMET0   ZERATOR	SQUEEZIE   DOMINGO    GOTAGA   HUGODECRYPTE   SARDOCHE   PONCE   ANTOINEDANIEL    ETOILES   LOCKLEAR   PAULETA_TWITCH    JLAMARU   SAMUELETIENNE
// WANKILSTUDIO   JOUEUR_DU_GRENIER   BICHOUU_   CHOWH1  ALPHACAST   LESTREAM   SHAUNZ   JOLAVANILLE    SOLARY 


//variable aleatoire pour les gros streamer r
//retourne un chiffre 
//$grosS = array($AMINEMATUE,$OTPLOL_,$ZERATOR,$KAMET0,$JLTOMY,$SQUEEZIE,$DOMINGO,$SARDOCHE,$GOTAGA,$HUGODECRYPTE,
//$MISTERMV,$ANTOINEDANIEL,$PONCE,$ETOILES,$JLAMARU,$LOCKLEAR,$PAULETA_TWITCH,$WANKILSTUDIO,$JOUEUR_DU_GRENIER,$BICHOUU_,
//$SAMUELETIENNE,$ALPHACAST,$LESTREAM,$SHAUNZ,$JOLAVANILLE,$LESTREAM);
//$aleatoireS = rand('0', count($grosS));
// <?php echo $grosS[$aleatoireS] 



//aleatoire en tableau;
//$stream[$AMINEMATUE] = 1;
/*
$stream[$OTPLOL_] = 1;
$stream[$ZERATOR] = 1;
...
$stream[$SARDOCHE] = 1;
$stream[$GOTAGA] = 1;
$stream[$HUGODECRYPTE] =1;
$stream[$MISTERMV] = 1;
$stream[$ANTOINEDANIEL] = 1;

$stream[$PONCE] = 1;
$stream[$ETOILES] = 1;
$stream[$JLAMARU] = 1;
$stream[$LOCKLEAR] =1;
$stream[$PAULETA_TWITCH] = 1;
$stream[$WANKILSTUDIO] = 1;

$stream[$JOUEUR_DU_GRENIER] = 1;
$stream[$BICHOUU_] = 1;
$stream[$SAMUELETIENNE] = 1;
$stream[$ALPHACAST] =1;
$stream[$LESTREAM] = 1;
$stream[$SHAUNZ] = 1;

$stream[$JOLAVANILLE] = 1;
$stream[$SHAUNZ] = 1;
*/
?>
<?php
...
$sql = "SELECT id, liens_stream FROM g_streamer WHERE id=1";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
        $stream= "" . $row["liens_stream"] . "";
    }
} else {
    echo "0 results";
}

$sql = "SELECT id, liens_stream FROM g_streamer WHERE id=2";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
        $stream1= "" . $row["liens_stream"] . "";
    }
} else {
    echo "0 results";
}

$sql = "SELECT id, liens_stream FROM g_streamer WHERE id=3";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
        $stream2= "" . $row["liens_stream"] . "";
    }
} else {
    echo "0 results";
}

$sql = "SELECT id, liens_stream FROM g_streamer WHERE id=4";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
        $stream3= "" . $row["liens_stream"] . "";
    }
} else {
    echo "0 results";
}

$sql = "SELECT id, liens_stream FROM g_streamer WHERE id=5";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
        $stream4= "" . $row["liens_stream"] . "";
    }
} else {
    echo "0 results";
}

$sql = "SELECT id, liens_stream FROM g_streamer WHERE id=6";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
        $stream5= "" . $row["liens_stream"] . "";
    }
} else {
    echo "0 results";
}

$sql = "SELECT id, liens_stream FROM g_streamer WHERE id=7";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
        $stream6= "" . $row["liens_stream"] . "";
    }
} else {
    echo "0 results";
}

$sql = "SELECT id, liens_stream FROM g_streamer WHERE id=8";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
        $stream7= "" . $row["liens_stream"] . "";
    }
} else {
    echo "0 results";
}

$sql = "SELECT id, liens_stream FROM g_streamer WHERE id=9";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
        $stream8= "" . $row["liens_stream"] . "";
    }
} else {
    echo "0 results";
}

$sql = "SELECT id, liens_stream FROM g_streamer WHERE id=12";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
        $stream9= "" . $row["liens_stream"] . "";
    }
} else {
    echo "0 results";
}
